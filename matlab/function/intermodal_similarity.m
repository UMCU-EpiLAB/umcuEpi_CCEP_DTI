function [J,R] = intermodal_similarity(JI, JI_expected, ratio)
% INPUT:
...
scatter(w,ratio(1:5),40,'o','k') % scatter the ecog patients
hold on
v = [0.74 0.7 0.7 0.7 0.7 0.6 0.7 0.67]; % make a XJitter such that the individual points do not overlap
scatter(v,ratio(6:13),400,'.','k','LineWidth', 1.8,'MarkerFaceAlpha',1,'MarkerEdgeAlpha',1) % scatter the seeg patients
hold on 
yline(0)
ylim([-1.1 1.1])
xlim([-1 2])
