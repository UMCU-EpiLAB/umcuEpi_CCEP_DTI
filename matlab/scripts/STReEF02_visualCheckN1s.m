...
% load detected CCEP data, visual check CCEPs
...
dataBase = load_detected_data(dataBase,myDataPath,cfg);
...
if size(dataBase(subj).ccep,2)>1
    runs = {dataBase(subj).ccep(:).run_label};
    string = [repmat('%s, ',1,size(runs,2)-1), '%s'];
    substring = input(sprintf(['Choose run: ',string,'\n'],runs{:}),'s');
    run = find(contains({dataBase(subj).ccep(:).run_label},substring));
...
if exist(fullfile(myDataPath.input_dev, dataBase(subj).sub_label, ...
        ...
        dataBase(subj).ccep(run).run_label,'_N1sChecked_new_rater.mat']),'file')
    
   dataBase(subj).ccep(run).ccep = load(fullfile(myDataPath.input_dev, ...
       dataBase(subj).sub_label,dataBase(subj).ses_label, dataBase(subj).ccep(run).run_label,...
        [dataBase(subj).sub_label, '_', dataBase(subj).ses_label,'_',...
        dataBase(subj).metadata(run).task_label,'_',...
        dataBase(subj).ccep(run).run_label,'_N1sChecked_new_rater.mat']));   
end
...
if any(strcmp(fieldnames(dataBase(subj).ccep(run)),'ccep'))
    if any(contains(fieldnames(dataBase(subj).ccep(run).ccep),'checkUntilStimp'))
        endstimp = dataBase(subj).ccep(run).ccep.checkUntilStimp;
    else
        endstimp = 0;
    end
else
...
for subj = 1:size(dataBase,2)
    for run = 1:size(dataBase(subj).ccep,2)
        ...
        start_filename = strfind(dataBase(subj).ccep(run).ccep.dataName,'/');
        stop_filename = strfind(dataBase(subj).ccep(run).ccep.dataName,'_ieeg');

        fileName = [dataBase(subj).ccep(run).ccep.dataName(start_filename(end)+1:stop_filename-1),'_N1sChecked_new_rater.mat'];

        ccep = dataBase(subj).ccep(run).ccep;
        ccep.dataName = dataBase(subj).ccep(run).ccep.dataName;
        ccep.ch = dataBase(subj).ccep(run).ccep.ch;
        ccep.cc_stimchans = dataBase(subj).ccep(run).ccep.cc_stimchans;
        ccep.cc_stimsets = dataBase(subj).ccep(run).ccep.cc_stimsets;
...
% housekeeping
clear ccep targetFolder run subj fileName start_filename stop_filename