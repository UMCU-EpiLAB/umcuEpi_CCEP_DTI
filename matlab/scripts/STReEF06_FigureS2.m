...
NS = visual_topology_predictor(dataBase(nSubj).topology.node_proximity, dataBase(nSubj).topology.degree_EC, dataBase(nSubj).soz_select,count);

end
saveas(NS,fullfile(targetFolder,'correlation np effective grid'),'epsc') % save the figure for further processing with Adobe Illustrator
...
saveas(NS,fullfile(targetFolder,'correlation np effective seeg 1'),'epsc') % save the figure for further processing with Adobe Illustrator
...
saveas(NS,fullfile(targetFolder,'correlation np effective seeg 2'),'epsc') % save the figure for further processing with Adobe Illustrator
