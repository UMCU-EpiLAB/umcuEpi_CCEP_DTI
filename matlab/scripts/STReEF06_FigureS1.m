...
NS = visual_topology_predictor(dataBase(subj).topology.node_proximity, dataBase(subj).topology.degree_SC, dataBase(subj).tb_electrodes,dataBase(subj).elec_include, i);

end
saveas(NS,'correlation np structural grid','epsc') % save the figure for further processing with Adobe Illustrator

i = 0;
for subj = 6:11 %eeg part 1 % plot in 3 parts to get the right dimensions
