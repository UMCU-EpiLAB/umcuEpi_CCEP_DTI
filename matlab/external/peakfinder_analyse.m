function varargout = peakfinder_analyse(x0, sel, thresh, extrema, includeEndpoints, interpolate,dataBase,subj,run,cfg,tt,chan,stimp)
%Zie voor werking document susanne
...
%if nargout == 0
    if isempty(peakInds)
        ...
        hold on
        tt_new=tt(find(tt>0,1):find(tt>0.5,1));
        h2=scatter(tt_new(peakInds),peakMags, 50, [0.4940 0.1840 0.5560], 'filled','DisplayName','from peakfinder');
    end
%else
