function V = visual_networks(matrix_m1, matrix_m2, sublabel)
% INPUT:
...
map_stim = [1 1 1;0.99 0.38 0.22];
colormap(ax2,map_umc2)
daspect([1 1 1])

siz = size(matrix_m2,1)/100;
t = text(ax2,25*siz,-15*siz,sprintf('%s',sublabel));
t.FontSize = 16;
set(gca,'XTick',[])
set(gca,'YTick',[])
xlabel(sprintf('Electrode area 1-%.2g',size(matrix_m2,1)),'Fontsize',f)
...
s2 = title(ax1,'Structural','Fontsize',f);
s3 = title(ax3,'Union and Intersection','Fontsize',f) 
end
