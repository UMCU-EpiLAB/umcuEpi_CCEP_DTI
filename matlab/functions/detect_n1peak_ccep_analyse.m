function [dataBase] = detect_n1peak_ccep_analyse(dataBase, cfg,chan,stimp)
...
for subj = 1:length(dataBase)
   ...
   % doe deze voor de peakfinder analyse
                    %[all_sampneg, all_amplneg] = peakfinder_analyse(new_signal(find(tt>0,1):find(tt>0.5,1)),sel,[],-1,[],[],dataBase,subj,run,cfg,tt,chan,stimp); 
                    %[all_samppos, all_amplpos] = peakfinder_analyse(new_signal(find(tt>0,1):find(tt>0.5,1)),sel,[],1,[],[],dataBase,subj,run,cfg,tt,chan,stimp); 
                   ...
                   %%
                    ...
                    hold on
                    %plot(tt,new_signal)
                    h2=scatter(tt(temp_n1_peaks_samp),temp_n1_peaks_ampl, 50, [0.4940 0.1840 0.5560], 'filled','DisplayName','from peakfinder');
                    delete(h2)
                    hold on
                    %close(f1)
                    ...
                    h3=scatter(tt(n1_peak_sample),n1_peak_amplitude, 50, [0.4940 0.1840 0.5560], 'filled','DisplayName','biggest peak');
                    delete(h3)
                    hold on
...
%                        n1_peak_amplitude = NaN;   
%                     end
%                     h31=scatter(tt(n1_peak_sample),n1_peak_amplitude, 50, [0.4940 0.1840 0.5560], 'filled','DisplayName','positive');
%                     delete(h31)
                   ...
                   h4=scatter(tt(n1_peak_sample),n1_peak_amplitude, 50, [0.4940 0.1840 0.5560], 'filled','DisplayName','saturated peak?');
                   delete(h4)
                   hold on
                   ...
                   h5= scatter(tt(n1_peak_sample),n1_peak_amplitude, 50, [0.4940 0.1840 0.5560], 'filled','DisplayName','big enough peak?');
                   delete(h5)
                end
