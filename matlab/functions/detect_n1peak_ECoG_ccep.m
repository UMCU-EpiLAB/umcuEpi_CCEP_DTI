...
%   standard deviation of 50 uV. amplitude_thresh of 2.6 recommended for
%   sensitive algorithm (50 uV * 2.6 = 130 uV).
% - n1_peak_range
...
%   End point of 100 ms recommended for sensitive algorithm 
...
% For a sensitive algorithm, the following 
% parameters are advised: (see validation matrices for performances with
% other parameters e.g. if you want a very conservative algorithm)

% - amplitude threshold of 130 uV (minSD * threshold = 50 uV * 2.6)
%   recommended
...
for subj = 1:length(dataBase)
    ...
    for run = 1:size(dataBase(subj).metadata,2)
        ...
        bad_ch = find(strcmp(dataBase(subj).metadata(run).tb_channels.status,'bad')); % this is also when in silicon is yes (so on top of each other)
