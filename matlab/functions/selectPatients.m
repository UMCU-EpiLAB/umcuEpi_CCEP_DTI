...
for subj = 1:size(cfg.sub_label,2)
    ...
    files = dir(fullfile(myDataPath.input,['sub-' cfg.sub_label{subj}]));
    ...
    if size(sesfiles,1) <1
        error('Pathname %s does not contain any files',fullfile(myDataPath.input,['sub-' cfg.sub_label{subj}]))
...
for subj = 1:size(cfg.sub_label,2)
    files = dir(fullfile(myDataPath.input,['sub-' cfg.sub_label{subj}],cfg.ses_label{subj},'ieeg'));
